...
/**
 * @file Pricing.tsx
 * @description Monolithic Financial Structure with Psychological Pricing logic.
...
const pricingPlans = [
  {
    ...
    region: "SOUTH ASIA",
    countries: "Pakistan • India • Bangladesh",
    price: 7999, // UPDATED TO CLIENT SPEC
    details: ["Full Labour Approval", "E-Visa Processing", "Medical & Fingerprint Support", "Emirates ID Issuance"]
  },
  {
    cat: "Category 02",
    region: "GLOBAL",
    countries: "All Other Nationalities",
    price: 5999, // UPDATED TO CLIENT SPEC
    details: ["Worldwide Enrollment", "Government Quota Liaison", "Status Change Assistance", "Full Residency Stamping"]
  }
];
    ...
    <section id="pricing" className="relative pt-0 pb-12 md:pb-24 lg:pb-32 bg-white px-6 overflow-hidden border-t border-black/[0.02]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12 md:mb-20">
          ...
          <div className="mt-8 md:mt-10 w-12 h-[1px] bg-black/10 mx-auto" />
        </div>

        {/* DUAL MONOLITH SYSTEM: MOBILE STACKING FIXED */}
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-0 border border-black/[0.03] rounded-sm overflow-hidden shadow-sm">
          {pricingPlans.map((plan, i) => (
            <motion.div
              key={i}
              variants={itemVars}
              ...
              viewport={{ once: true }}
              className={`relative p-8 md:p-16 lg:p-24 flex flex-col group transition-all duration-1000 ${
                i === 0 ? "lg:border-r border-black/[0.03]" : ""
              } bg-white hover:bg-[#F8FAFC]`}
            >
              <div className="flex justify-between items-start mb-12 md:mb-20">
                <div>
                  <span className="text-[10px] font-black text-[#B38B59] tracking-[0.4em] uppercase block mb-2">{plan.cat}</span>
                  <h3 className="text-2xl md:text-4xl font-heading font-black text-[#0F172A] tracking-tighter uppercase">{plan.region}</h3>
                </div>
                <ShieldCheck size={24} className="text-[#B38B59] opacity-20 group-hover:opacity-100 transition-opacity hidden sm:block" />
              </div>

              <div className="mb-10 md:mb-16">
                <p className="text-[9px] font-bold text-[#64748B] uppercase tracking-[0.3em] mb-4">Starting Investment</p>
                <div className="flex items-baseline gap-2 md:gap-4">
                  <span className="text-2xl md:text-4xl font-light text-black/10 font-heading">AED</span>
                  <h4 className="text-6xl md:text-8xl lg:text-[110px] font-heading font-black text-[#0F172A] leading-none tracking-tighter group-hover:text-[#B38B59] transition-colors duration-700 tabular-nums">
                    <Counter value={plan.price} />
                  </h4>
                </div>
                <p className="text-[10px] md:text-[11px] font-bold text-[#0F172A]/40 uppercase tracking-widest mt-6 italic">{plan.countries}</p>
              </div>

              <div className="space-y-4 mb-12 md:mb-20">
                {plan.details.map((detail, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#B38B59] flex-shrink-0" />
                    <span className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-[#0F172A] opacity-70 group-hover:opacity-100 transition-opacity">{detail}</span>
                  </div>
                ))}
              </div>

              <button 
                onClick={() => onSelect?.(plan.region.includes("SOUTH") ? "Pakistan" : "Other / Worldwide", `${plan.cat} - ${plan.price} AED`)}
                className="mt-auto flex items-center justify-between w-full border border-black/10 p-5 md:p-6 group/btn hover:bg-[#0F172A] transition-all duration-500 rounded-sm cursor-pointer"
              >
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#0F172A] group-hover/btn:text-white transition-colors">Select Category</span>
                <ArrowRight size={16} className="text-[#B38B59] group-hover/btn:translate-x-2 transition-transform" />
              </button>
        ...
        {/* REGULATORY NOTICE: SPACING CORRECTED */}
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="mt-12 md:mt-24 max-w-4xl mx-auto border border-dashed border-[#B38B59]/30 p-8 md:p-12 text-center relative bg-[#F8FAFC]/30">
